...
uint8_t ler_nivel_percentual()
{
    ...
    if (adc_nivel <= ADC_MIN)
        return 0;
    if (adc_nivel >= ADC_MAX)
        return 100;
...
{
    if (nivel <= lim_min)
    {                         // Se o nivel for menor que o limite minimo
        gpio_put(LED_RED, 1); // Liga o LED vermelho
        gpio_put(LED_GREEN, 0);
    }
    else if (nivel < 50)
    { // Se o nivel for menor que 50
        gpio_put(LED_RED, 1);
        gpio_put(LED_GREEN, 1); // Liga a cor amarela (vermelho + verde)
    }
    else
    { // Se o nivel for maior que 50
        gpio_put(LED_RED, 0);
...
void verificar_buzzer(uint8_t nivel, uint8_t lim_min, uint8_t lim_max)
{
    ...
    { // Se o nivel for maior que o limite maximo
        pwm_set_enabled(buzzer_slice, true);
        sleep_ms(100);
        pwm_set_enabled(buzzer_slice, false);
...
void atualizar_matriz_leds(uint8_t nivel)
{
    ...
    if (faixa > 4)
        faixa = 4; // Limita o numero de faixas
...
int main()
{
    ...
    while (true)
    {
        ...
        // Controle da bomba
        if (acionar_bomba || nivel < limite_minimo)
        {                          // Se o botão A for pressionado e o nivel for menor que o limite maximo
            gpio_put(RELE_PIN, 1); // Liga a bomba por pulso
            sleep_ms(200);
            gpio_put(RELE_PIN,0);
            bomba_ligada = true;   // Indica que a bomba esta ligada
            while (ler_nivel_percentual() < limite_maximo)
            { // Enquanto o nivel for menor que o limite maximo
                sleep_ms(200);
            }
            gpio_put(RELE_PIN, 1); // Desliga a bomba por pulso
            sleep_ms(200);
            gpio_put(RELE_PIN,0);
